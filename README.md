...
> **About StrongAI:** C4-Auto was created as part of the [StrongAI](https://github.com/jonverrier) project. StrongAI is both a statement of belief in AI and a brand that helps community and performance gyms deliver differentiated experiences through AI. The "StrongAI" name appears in generated file names (`README.StrongAI.Component.md`), sentinel markers (`// ===Start StrongAI Generated Comment===`), and datestamps (`<!-- Generated by StrongAIAutoDoc YYYYMMDD -->`). The `@jonverrier/prompt-repository` package provides the LLM prompt management and chat driver infrastructure used by this tool.
...
**Naming conventions:** This codebase follows a C++-influenced naming style that is less common in TypeScript but is used consistently throughout:

- **`E` prefix on enums** – e.g. `ETimeWindow`, `EC4DiagramType`. A reminder that the type is an enum (vs interface or class).
- **`I` prefix on interfaces** – e.g. `IFileReader`, `IDocGenOptions`. Follows the convention popularised by C# and older TypeScript style guides.
- **`k` prefix on enum values** – e.g. `ETimeWindow.kOneWeek`, `EC4DiagramType.kComponent`. A C++ convention meaning "constant"; used here to distinguish enum members from PascalCase types at a glance.

If you are used to idiomatic TypeScript (where enum values are typically PascalCase or SCREAMING_SNAKE_CASE), these prefixes may look unfamiliar—but they are applied consistently everywhere in the project.

**LLM model:** The tool calls `EModel.kLarge` via `EModelProvider.kOpenAI`, which maps to **gpt-4o** in the `@jonverrier/prompt-repository` library. This is used for both module header generation and C4 diagram generation.
